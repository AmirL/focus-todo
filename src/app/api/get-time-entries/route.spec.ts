...
function makeRequest() {
  return new NextRequest(new URL('http://localhost:3000/api/get-time-entries'), {
    method: 'POST',
    body: JSON.stringify({}),
  } as never);
}

describe('POST /api/get-time-entries', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(validateUserSession).mockResolvedValue({ user: { id: 'user-1' } } as never);
    mockFrom.mockReturnValue({ where: mockWhere });
  ...
  it('returns time entries for authenticated user', async () => {
    const entries = [{ id: 1, taskId: 5, userId: 'user-1' }];
