...
# Check E2E prerequisites (Cypress loads credentials from .env.local via cypress.config.ts)
if [ ! -f ".env.local" ]; then
  echo ""
  echo "ERROR: .env.local not found."
  echo "  Coverage requires NEXT_PUBLIC_TEST_EMAIL and NEXT_PUBLIC_TEST_PASSWORD in .env.local."
  echo ""
  exit 1
fi
...
if [ -f "$MERGED_DIR/coverage-summary.json" ]; then
  echo ""
  echo "== Coverage Summary (unit + e2e) =="
  node -e "
